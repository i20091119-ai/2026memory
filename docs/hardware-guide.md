...
> **정식 게임기(라즈베리파이 5 + USB 인코더, 책상 2대 + 흰색 버튼)** 는
> [`raspberry-pi-setup.md`](raspberry-pi-setup.md) 를 따른다. 아래 1~6절은 파일럿 구성
> (우노 Q + App Lab 브리지)의 기록이며, 버튼 배선(2절)·LED(3절)·전원(4절)·스피커(5.6)·
> 문제 해결(6절)은 정식 게임기에도 그대로 쓰인다. 정식 구성에서 달라지는 것은 7절에 모았다.
...
## 7. 정식 게임기에서 달라지는 것 (라즈베리파이 5 + 인코더)

설치 절차 전체는 [`raspberry-pi-setup.md`](raspberry-pi-setup.md). 여기에는 파일럿과
**다른 점**만 적는다.

| | 파일럿 (우노 Q) | 정식 (라즈베리파이 5) |
|---|---|---|
| 버튼 읽기 | STM32 스케치 → App Lab 브리지 → WebSocket | **USB 인코더** → 게임패드 (코드·브리지 없음) |
| 버튼 수 | 책상당 4 (빨·노·초·파) | 책상당 **5** — 흰색 추가 (2인 대결 도전) |
| 책상당 두뇌 | 우노 Q 1대 | 파이 **1대가 책상 2대** (모니터 2대를 3840×1080 한 창) |
| 화면 | 포터블 모니터, USB-C 허브 | 27" FHD ×2, HDMI 직결 |
| 소리 | 허브의 3.5mm | USB 사운드카드 → Britz BR-1000A2 (좌우 책상 = 좌우 채널) |
| 부팅 | PD 협상 타이밍 문제 있음 | 전원 들어오면 무조건 부팅 |
| 감시 스크립트 | 필요 (브리지가 죽음) | 불필요 |
| GND | 데이지체인 (단일 고장점) | 버튼마다 2선 (신호+GND) — 데이지체인 없음 |

### 7.1 흰색 버튼

책상마다 100파이 돔 버튼 하나를 더 단다 — 색 버튼 4개 오른쪽 끝, 같은 간격.
게임 입력이 아니라 **옆 책상에 도전을 걸고 받는** 전용 버튼이다. LED 는 다른 버튼과
같이 12V 병렬. 인코더 입력은 K5(책상 1) / K10(책상 2).

### 7.2 인코더 배선표

`raspberry-pi-setup.md` 5절과 같다. K1~K5 = 책상 1 빨·노·초·파·흰, K6~K10 = 책상 2.
마이크로스위치 COM·NO 에 2선 케이블. 책상 2 는 십색선으로 2m 연장.

### 7.3 책상 사이를 지나는 것

HDMI(모니터 2), 버튼 신호선 5쌍, 스피커선(패시브 짝), 전원선(모니터 2·LED 어댑터).
에이프런에 **지름 30mm 통로 하나**면 된다.
