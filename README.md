...
100파이 아케이드 버튼 5개(빨강·노랑·초록·파랑 + 흰색)로 플레이하는 기억력 게임.
정식 게임기는 **라즈베리파이 5 한 대가 책상 2대(모니터 2대)**를 맡고, 두 사람이
흰색 버튼으로 서로 도전해 **2인 대결**을 할 수 있다.
...
옆 책상에 사람이 있을 때 **흰색 버튼**을 누르면 그쪽 화면에 "도전에 응하시겠습니까?"가
10초 동안 뜬다. 상대가 **흰색을 누르면 수락**, 하던 걸 계속하면(다른 색 버튼을 누를 때마다
남은 시간이 2.5초씩 줄어든다) 그냥 지나간다. 도전은 놀고 있지 않을 때(타이틀·선택·
결과 화면)만 걸 수 있다.

수락하면 둘 다 하던 게임을 멈추고 대결로 들어간다:

- **게임 고르기** — 두 화면에 4색 카드. 먼저 누르는 사람이 정한다 (8초 지나면 혼합).
- **같은 문제가 동시에** 나온다. 먼저 전부 맞힌 사람이 1점. 틀리면 그 라운드는 상대 것,
  둘 다 틀리면 무효. 상대가 몇 개째 맞히는지 내 화면 위에 점으로 보인다.
- 1·1·2·2·3·3·4·4개 기억, 8라운드. **먼저 5점**을 따면 승리. 8라운드 뒤 동점이면 서든데스.
- 어느 쪽이든 빨강+파랑 2초 홀드로 대결을 끝낼 수 있다.
...
키보드 **1 2 3 4 5** = 빨강·노랑·초록·파랑·흰색. 화면의 버튼·카드를 마우스나 터치로 직접 눌러도 된다.
`?seats=2` 를 붙이면 화면이 좌우 두 좌석으로 갈리고 오른쪽 좌석은 **Q W E R T** 다 (대결을 혼자 시험해 볼 수 있다).
...
| 옆 사람에게 도전 | **흰색** 버튼 (타이틀·선택·결과 화면에서). 상대도 흰색을 누르면 대결 시작 |
| 홈으로 나가기 | **빨강 + 파랑을 2초 동안 함께** (화면 위쪽에 게이지가 찬다). 브라우저에서는 **ESC** 로도 된다 |
| 운영 기록 보기 | 타이틀에서 **노랑 + 초록을 3초 동안 함께**. 브라우저에서는 **0** 키 (아래 "운영 기록" 참조) |
| 입력 제한시간 | 없음 |
...
판마다 한 줄씩 **플레이 기록**이 남는다 — 시각, 게임 종류, 어디까지 갔나,
결과(완주/게임오버/중도이탈), 소요 시간, 이어하기·오답 횟수. 개인정보는 없고
기기의 localStorage 안에만 쌓이며 어디로도 보내지 않는다 (`js/playlog.js`).

타이틀에서 **노랑 + 초록을 3초** 누르면 집계 화면이 뜬다.

| 버튼 | |
|---|---|
| 노랑 | 기간 바꾸기 — 이번 달 → 지난 달 → 오늘 → 전체 |
| 초록 | 그 기간의 **CSV 저장** (다운로드 폴더, 엑셀에서 바로 열림) |
| 파랑 두 번 | 기록 전부 지우기 (실수 방지로 5초 안에 두 번) |
| 빨강 | 닫기 (90초 방치해도 닫힌다) |

보이는 것: 플레이 판 수 · 완주 수와 완주율 · 중도 이탈 · 한 판 소요 시간(중간값) ·
게임별 인기와 완주율 · **몇 개 기억까지 갔나**(탈락 곡선) · 시간대별 플레이.
"판"은 사람 수가 아니라 게임을 고른 횟수다 — 한 사람이 두 게임을 하면 2판이다.

키오스크에서 CSV 는 `~/Downloads/torus-log-2026-09.csv` 처럼 떨어지므로
SSH 나 USB 로 꺼내면 된다. 화면을 사진으로 찍어 가도 충분하게 만들어 두었다.
...
| `?seats=2` | 두 좌석(책상 2대) 모드. 화면 가로세로비가 2.6 이상이면 저절로 켜진다 |
| `?debug=1` | `window.__torus` 로 좌석·대결 상태를 노출 (자동 검증용) |
| `?bridge=0` | "버튼 미연결" 경고를 끈다 (인코더 구성에서 브리지는 원래 없다) |
...
```bash
node --test          # 72개 테스트 (게임 규칙 + 대결 규칙 + 모듈 정합성 + 플레이 기록 집계)
```
...
├── state.js            # 상태 머신 메인 루프 (좌석 하나의 혼자 놀기)
├── arena.js            # 두 좌석을 묶는 심판 — 도전 신청/수락, 대결 진행
├── versus.js           # 대결 규칙 순수 함수 (테스트 대상)
├── main.js             # 부트스트랩·좌석 생성·키오스크 설정
├── input.js            # KeyboardInput / GamepadInput(인코더) / SocketInput 추상화
...
├── storage.js          # 게임별 완주 횟수 저장/로드
├── playlog.js          # 플레이 기록(판마다 한 줄) + 집계·CSV — 운영자 화면의 재료
├── util.js             # DOM·대기 도우미 (중도 이탈 신호 포함)
└── scenes/             # title, select(게임 선택), intro, present, recall,
                        #   feedback, gameover, allclear, attract, admin(운영 기록),
                        #   versus(도전 프롬프트·대결 화면), hud, strip
...
├── install-kiosk.sh    # 게임 설치 + 자동시작 등록 (파이/우노 Q 자동 감지)
├── pi-kiosk.sh         # 라즈베리파이: 모니터 2대 배치·소리·3840×1080 창 띄우기
├── bridge-watch.sh     # (우노 Q) 버튼 브리지가 죽으면 진단·자동 복구
└── start-all.sh        # (우노 Q) 서버+브리지+전체화면 한 번에 띄우기
...
- 저장: localStorage — 게임별 완주 횟수(`torus-memory.clears.v2`)와
  플레이 기록(`torus-memory.log.v1`, 최대 5,000줄). 하이스코어는 없다 —
  부스라 매번 다른 사람이 오고 게임에 끝이 있어 의미가 없다.
...
[`docs/hardware-guide.md`](docs/hardware-guide.md), 버튼 브리지는
